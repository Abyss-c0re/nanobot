...
export PATH="/opt/nanobot/bin:/usr/local/bin:$PATH"
...
if [[ -f /auth/in.nbundle ]]; then
  auth_bundle_import /auth/in.nbundle "${AUTH_BUNDLE_PASS:-}"
elif [[ -n "${AUTH_BUNDLE_IN:-}" && -f "${AUTH_BUNDLE_IN}" ]]; then
  auth_bundle_import "$AUTH_BUNDLE_IN" "${AUTH_BUNDLE_PASS:-}"
fi
  ...
  peer|serve|server)
    if [[ -n "${PROMPT:-}" ]]; then
      ...
      for _ in $(seq 1 40); do
        curl -fsS "http://127.0.0.1:${NANOBOT_PORT}/peer/v1/health" >/dev/null 2>&1 && break
        sleep 0.25
      done
      bash /opt/nanobot/modules/mcp_ready.sh
    ...
    ;;
  mcp)
    # stdio MCP — export on stop still via EXIT if shell remains parent
